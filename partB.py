...
def find_gaps(hist, min_width=60,multiplier=0.15):
    treshold = max(np.percentile(hist, 5), np.max(hist) * multiplier, 1)
...
def get_region(gaps,hist, min_length = 15 ):
    regions = []
    r_start = 0
    padding = 20
    limit = len(hist)
...
#identify_table
def contains_line(hist, spike_ratio=0.98, min_lines=5):
   threshold = np.max(hist) * spike_ratio
   #go through all value in histogram and only keep value that equal or bigger than threshold
   spikes = [val for val in hist if val >= threshold]
   
   return len(spikes) >= min_lines
...
def extract_paragraph(image,m1=0.15,m2=0.02):
    all_para_imgs = []
    col_imgs = extract_column(image,m1)
    ...
    for i, col_img in enumerate(col_imgs):
        ...
        gaps = find_gaps(hori_hist,30,m2)
        ...
        for para_img in para_imgs:
            para_hist = np.sum(para_img == 0, axis=1)
            if not is_diagram(para_img) and not contains_line(para_hist):
        ...
        try:
            ...
            match choice:
                ...
                case 3:
                    ...
                    break
                case 4:
                    extract_paragraph("resources/Converted_Paper/004.png",0.3,0.02)
                    break
                case 5:
                ...
                case 7:
                    ...
                    break
                case 8:
                    extract_paragraph("resources/Converted_Paper/008.png",0.3,0.02)
                    break
                case _:
